...
.block-container { padding: 0 1rem 3rem !important; max-width: 1300px !important; }

/* HEADER */
.aero-header { text-align:center; padding:2rem 1rem 1.4rem; }
.aero-header h1 {
...
.aero-header .sub  { font-size:.88rem; color:var(--text-muted); letter-spacing:1.5px; margin-bottom:.2rem; }
.aero-header .credit { font-family:'JetBrains Mono',monospace; font-size:.64rem; color:var(--text-dim); }
.aero-divider {
  height:1px;
  background: linear-gradient(90deg,transparent,var(--accent) 40%,var(--purple) 60%,transparent);
  opacity:.35; max-width:480px; margin:1.1rem auto 0;
}
...
.cat-inner {
  ...
  gap:.4rem; padding:.15rem .1rem .3rem;
  width:max-content;
}
.cat-btn {
  ...
  padding:.42rem 1rem; white-space:nowrap;
  cursor:pointer; transition:all .18s ease;
  -webkit-tap-highlight-color:transparent;
  display:inline-block; text-decoration:none;
}
.cat-btn:hover {
  background:rgba(0,200,240,.08); color:var(--accent);
  border-color:rgba(0,200,240,.45);
  box-shadow:0 0 12px rgba(0,200,240,.1);
}
.cat-btn.active {
  background:rgba(0,200,240,.12); color:var(--accent);
  border-color:var(--accent);
  box-shadow:0 0 16px rgba(0,200,240,.18);
}
...
.section-label .sources { color:var(--text-dim); font-size:.58rem; margin-left:.3rem; }
...
.news-card:hover {
  transform:translateY(-2px); border-color:rgba(0,200,240,.3);
  box-shadow:0 6px 24px rgba(0,200,240,.06);
}
.card-body { padding:1rem 1.1rem .85rem 1.25rem; }
.card-top {
...
.cat-badge {
  ...
  padding:.1rem .48rem; flex-shrink:0;
}
.src-badge {
...
.sent-badge {
  ...
  padding:.1rem .42rem; border-radius:2px; flex-shrink:0;
}
...
.news-title { font-size:.93rem; font-weight:700; color:var(--text); line-height:1.45; margin-bottom:.55rem; }
.news-desc  { font-size:.81rem; color:var(--text-muted); line-height:1.62; margin-bottom:.75rem; }
.card-meta {
...
/* DETAIL PANEL */
.detail-panel {
  border-top:1px solid rgba(0,200,240,.12); background:rgba(0,8,20,.5);
...
.detail-text { font-size:.82rem; color:var(--text-muted); line-height:1.7; }
.detail-foot { margin-top:.45rem; font-family:'JetBrains Mono',monospace; font-size:.55rem; color:var(--text-dim); }

/* AI LOADING */
.ai-loading-wrap {
  border-top:1px solid rgba(0,200,240,.14);
  background:linear-gradient(135deg,rgba(0,200,240,.025),rgba(96,32,204,.02));
  padding:.9rem 1.1rem .9rem 1.25rem;
}
.ai-load-title {
  font-family:'JetBrains Mono',monospace; font-size:.6rem;
  font-weight:700; letter-spacing:2.5px; color:var(--accent);
  text-transform:uppercase; margin-bottom:.65rem;
  display:flex; align-items:center; gap:.45rem;
}
.ai-spinner {
  width:8px; height:8px; border-radius:50%;
  border:1.5px solid rgba(0,200,240,.2); border-top-color:var(--accent);
...
.ai-steps { display:flex; flex-direction:column; gap:.25rem; }
.ai-step-row {
  display:flex; align-items:center; gap:.45rem;
  font-family:'JetBrains Mono',monospace; font-size:.61rem; color:var(--text-dim);
}
...
.ai-result-wrap {
  border-top:1px solid rgba(0,200,240,.14);
  background:linear-gradient(135deg,rgba(0,200,240,.03),rgba(96,32,204,.025));
  padding:.9rem 1.1rem 1rem 1.25rem;
}
.ai-result-header {
  display:flex; align-items:center; gap:.45rem;
...
@keyframes pulse-glow {
  ...
  50%      { opacity:.4; box-shadow:0 0 14px var(--accent); }
}
.ai-result-text { font-size:.83rem; color:var(--text); line-height:1.8; white-space:pre-wrap; }

/* EMPTY STATE */
.empty-wrap {
  text-align:center; padding:3rem 2rem;
  border:1px solid var(--border); border-radius:6px;
  margin:1.2rem auto; max-width:520px;
}
.empty-wrap h3 {
...
.empty-wrap p { font-size:.81rem; color:var(--text-dim); line-height:1.65; }

/* FOOTER */
.aero-footer {
  margin-top:2.5rem; padding:1.1rem;
  border-top:1px solid var(--border);
  text-align:center; font-family:'JetBrains Mono',monospace;
  font-size:.61rem; color:var(--text-dim); line-height:1.9;
}
...
# ══════════════════════════════════════════════
# KATEGORI — HTML scroll horizontal
# ══════════════════════════════════════════════
...
def muat_data(kategori: str) -> dict:
    """Load berita multi-source dengan cache 1 jam per kategori."""
    initialize_news_cache()
    ...
    if kategori == "all":
        ...
        data = {kategori: ambil_berita_kategori(
            kategori=kategori,
            marketaux_key=marketaux_key,
            newsapi_key=newsapi_key,
            fmp_key=fmp_key,
...
SRC_LABEL = {"marketaux":"MX","newsapi":"NA","fmp":"FP","":"--"}
...
def deteksi_sumber(item: dict) -> str:
    sumber = (item.get("sumber","") or "").lower()
    if "fmp" in sumber or "financial" in sumber:
        return "FMP"
    raw = item.get("raw", {})
    if raw.get("api_token") or "marketaux" in str(raw):
        return "MX"
    return item.get("sumber","")[:12]
...
# Hitung sumber aktif
aktif_src = []
if marketaux_key: aktif_src.append("Marketaux")
if newsapi_key:   aktif_src.append("NewsAPI")
if fmp_key:       aktif_src.append("FMP")
...
if not items:
    ...
    src_str = " + ".join(aktif_src) if aktif_src else "-"
    st.markdown(f"""
    <div class="section-label">
      <span>FEED BERITA</span>
      <span class="count">{len(items)} artikel</span>
      <span class="sources">via {sx(src_str)}</span>
    </div>""", unsafe_allow_html=True)
        ...
        with cols_news[i % 2]:
                ...
                if st.button("AI ANALISIS", key=f"ai_{kid}", use_container_width=True):
                    ...
                    for idx, step in enumerate(AI_STEPS):
                        ...
                        for j, s in enumerate(AI_STEPS):
                            if j < idx: cls, icon = "done", "v"
                            elif j == idx: cls, icon = "active", ">"
                            else: cls, icon = "", "o"
            ...
            if st.session_state.show_detail.get(kid, False):
                ...
                <div class="detail-panel">
                  ...
                  <div class="detail-foot">Sumber: {src_lbl} | {sx(wkt)}</div>
                </div>""", unsafe_allow_html=True)
